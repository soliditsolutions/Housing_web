...
**Definición de "borrador"** (confirmada con el usuario, misma fecha): el generador de contratos (ítem #9, ver PROGRESO.md) produce un documento que un abogado revisa y legaliza antes de usarse — el sistema nunca certifica el contrato final. Esto fija el estándar de corrección para todo trabajo futuro sobre cláusulas/textos legales: realismo y ausencia de afirmaciones falsas, no certificación legal externa. Implementado el mismo día: `lib/contrato-pdf.tsx` + `GET /api/contratos/[id]/pdf` — plantilla fija (sin redacción libre de IA) que reutiliza exactamente las correcciones de esta revisión (renta del período faltante, Art. 21 Ley 18.101, Ley 21.719 vigente).

**Corrección — el borrador solo tiene sentido en estado `borrador`** (mismo día, hallazgo del usuario): la primera versión del botón "Descargar borrador (PDF)" se mostraba sin importar el estado del contrato. `FirmaSection` exige confirmar "ambas partes han firmado" antes de `activarContrato` (`borrador → vigente`), es decir, la firma real siempre ocurre fuera del sistema, antes de la activación. Ofrecer un PDF marcado "BORRADOR" para un contrato ya `vigente` es redundante en el mejor caso (el documento real ya existe y ya se firmó) y engañoso en el peor: el sistema no tiene forma de saber si un abogado ajustó algo entre el borrador generado y la firma real, así que un PDF regenerado podría no coincidir con lo firmado. Corregido en ambas capas (mismo criterio de defensa en profundidad que el resto del sistema): `panel/contratos/[id]/page.tsx` solo renderiza el botón si `estado === "borrador"`; `GET /api/contratos/[id]/pdf` devuelve `409 Conflict` si el contrato ya no está en borrador, para que nadie pueda forzar la descarga aunque la UI se manipule.
